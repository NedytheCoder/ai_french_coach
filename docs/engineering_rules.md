...
**Why:** The original codebase was French-only. This architecture deliberately undoes that assumption. Every deviation from this rule creates future migration work.
...
## Rule 5: Services Own Business Logic; Handlers Own HTTP

API handlers (`api/*.py`) do:
- Validate request data (via Pydantic models)
...
- Compute XP or level-up logic

Services (`services/*.py`) own all business logic.

**Why:** Testable business logic. Handlers can be tested with mock services; services can be tested without HTTP context.
...
2. Test an AI chat session with the new language pair
...
## Rule 7: No Comments That Describe What the Code Does

Write no comments that restate what the code already expresses through naming.
...
**Do write a comment when the WHY is non-obvious:**
```python
# Retry up to 5 times — Windows holds file locks after close()
for _ in range(5):
    try:
        os.unlink(tmp_path)
        break
...
**Why:** Comments that describe the what rot as the code changes. Comments that explain the why remain useful indefinitely.
...
- Fetch wrappers live in `lib/api.ts`, not inline in components
