...
function PassageCard({ passage, instructions }: { passage: string, instructions: string }) {
  return (
    <motion.div
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.98 }}
      >
        <span>{isLast ? "Listening Test" : "Next Question"}</span>
        <motion.div
          animate={{ x: [0, 5, 0] }}
...
export default function ReadingTestLevel() {
  ...
  const progress = ((currentIndex + (hasAnswered ? 1 : 0)) / questions.length) * 100
   ...
   useEffect(() => {
        const fetchReadingTest = async () => {
          try {
            const response = await fetch(`${API_BASE_URL}/reception/reading-placement-tests`)
  ...
  useEffect(() => {
    const fetchImmediateQuestionFeedback = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/reception/immediate-read-question-feedback`)
        const data = await response.json()
  ...
  const handleNext = () => {
    if (currentIndex < questions.length - 1) {
      ...
      // Navigate to listening test after last question
      router.push("/reception/tests/listen_test_level")
    }
  }
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50/30 to-teal-50/20 font-sans flex items-center justify-center">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="order-1">
              <PassageCard passage={currentQuestion.passage} instructions={currentQuestion.instructions} />
            </div>
